...
const Tag = () => {
  ...
  const showTags = () => {
    return tags.map((t, i) => {
      return (
        <button
          onDoubleClick={() => deleteTagConfirm(t.slug)}
  ...
  const showRemoved = () => {
    if (removed) {
      return <p className="text-danger">Tag successfully removed</p>;
    }
  };
  ...
  const newTagForm = () => (
    <form onSubmit={clickSubmit}>
      <div className="form-group">
        <label className="text-muted catsTags">Tags</label>
        <input
          type="text"
